...
// Process edit tool form
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $name = $_POST['name'];
    $icon = $_POST['icon'];
    $description = $_POST['description'];
    
    $stmt = $pdo->prepare("UPDATE tools SET name = ?, icon = ?, description = ? WHERE id = ?");
    $stmt->execute([$name, $icon, $description, $toolId]);
            ...
            <div class="card-body">
                <form method="post">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="name" name="name" value="<?php echo htmlspecialchars($tool['name']); ?>" required>
                    </div>
                    <div class="mb-3">
                        <label for="icon" class="form-label">Icon (Emoji or HTML code)</label>
                        <input type="text" class="form-control" id="icon" name="icon" value="<?php echo htmlspecialchars($tool['icon']); ?>" placeholder="e.g., 🎨 or <i class='fas fa-paint-brush'></i>" required>
                    </div>
                    <div class="mb-3">
